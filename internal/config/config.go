...
type StorageConfig struct {
	...
	LocalPath string `env:"STORAGE_LOCAL_PATH" envDefault:"/tmp/liteflow-storage"`
	OSS       OSSConfig
}

type OSSConfig struct {
	Endpoint  string `env:"OSS_ENDPOINT"`
	AccessKey string `env:"OSS_AK"`
	SecretKey string `env:"OSS_SK"`
	Bucket    string `env:"OSS_BUCKET"`
	RegionID  string `env:"OSS_REGION_ID"`
	RoleARN   string `env:"OSS_ROLE_ARN"`
}
