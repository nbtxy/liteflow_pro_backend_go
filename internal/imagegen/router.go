...
// ProviderRouter selects the appropriate image generation provider.
...
func (r *ProviderRouter) Register(p Provider) {
	...
	slog.Info("registered image generation provider", "name", p.Name())
}
...
func (r *ProviderRouter) Get(name string) (Provider, error) {
	...
	if !ok {
		return nil, fmt.Errorf("image generation provider not found: %s", name)
	}
	return p, nil
}
